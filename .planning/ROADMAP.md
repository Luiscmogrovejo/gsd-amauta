...
- [ ] **Phase 50: Party Mode Foundation** — `party_sessions` PG table + session state machine + persistent session memory (resumable from blackboard replay) (PARTY-01, PARTY-02). Plans 50-01 + 50-02 complete (2026-05-13): migration 021 (UP+DOWN) + PartySession Pydantic + 5 FROZEN transitions + post_finding() + list_findings() + resume() findings replay + 32 tests (SC3 two-agent ordering + SC4 daemon-restart sim). Plans 50-03 (CLI) + 50-04 (E2E) remain.
