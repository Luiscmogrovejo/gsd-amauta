...
**v3.2 The Federation** — SHIPPED 2026-05-14. 6 phases (48-53), 22 requirements, ~58 Phase 53 tests + 493 v3.2 accumulated tests across Phases 48-52.
...
- [x] **Phase 51: Party Mode Decisions + Operator CLI** — Structured decision records (propose/agree/dissent/block) + `gsd-amauta party status/inspect/kill` subcommands (PARTY-03, PARTY-04). Depends on 50. All 4 plans COMPLETE (2026-05-13): migration 023 + decision helpers + 11 Wave 1 tests + SC1 decision trail (5 tests) + SC3 dissent-no-rollback (4 tests) + party_session_cli.py status/inspect/kill subcommands + gsd-tools.cjs 9-action KNOWN_ACTIONS + tests/party-decisions-cli.test.cjs (10 tests) + tests/party-decisions-e2e.test.cjs (9 pass) + tests/party-decisions-canary.test.cjs (9 pass, NEVER SKIPS, Phase 50 byte-preservation + 13 cross-phase protected paths). 46 party-mode tests total (27 pytest + 19 Node). PARTY-03 + PARTY-04 fulfilled.
- [x] **Phase 52: Agent Compilation** — Canonical AGENT.yaml schema + `gsd-tools agents compile` symmetric with Phase 43 skill compiler + per-IDE alias tables + optional compile-time hydration bake (COMPILE-01, COMPILE-02, COMPILE-03, COMPILE-04). Plan 52-01 complete (2026-05-13): AgentDefinition Pydantic schema + agent-md-to-yaml.cjs converter + 24 tests. Plan 52-02 complete (2026-05-13): 17 canonical AGENT.yaml files generated + validated via load_agent_definition(). Plan 52-03 complete (2026-05-13): scripts/agent-compiler.cjs (compile/validate/listAgents + TARGET_MAPS 3 IDEs) + SC1 byte-match 17/17 PASS + 13 unit tests. Plan 52-04 complete (2026-05-13): gsd-tools.cjs case 'agents': dispatch (compile/validate/list + Usage banner) + bin/cli.cjs agents spawnSync branch + tests/gsd-tools-agents-cli.test.cjs (8 CLI integration tests, 8/8 pass). COMPILE-02 + COMPILE-03 CLI surface fulfilled. Plan 52-05 complete (2026-05-13): --hydrate wired (invokeHydration two-stage spawnSync + mergeHydration) + tests/agent-compiler-hydrate.test.cjs (6/6) + tests/phase-52-canary.test.cjs (11/11, NEVER SKIPS, PHASE_52_BASE=21438ae). COMPILE-04 fulfilled. Phase 52 COMPLETE — all COMPILE-01..04 requirements fulfilled.
- [x] **Phase 53: v3.1 Carry-Forwards** — Skill input/output schemas + installer upgrade/uninstall + MCP `amauta/bearings` + `amauta/agent-hydrate` + hydration auto-invoke at Task() spawn sites (POLISH-01..05). Plan 53-01 (POLISH-01) COMPLETE 2026-05-14: SkillFrontmatter input_schema + output_schema at positions 8+9 + validateJsonSchemaShape in skill-compiler.cjs + 13 Python tests + 11 Node tests. Plan 53-02 (POLISH-02) COMPLETE 2026-05-14: bin/init.cjs --upgrade (6 frozen steps, migration-delta, idempotent) + --uninstall (6 frozen steps, preservation contract) + mutual-exclusion guard + --dry-run + 8 integration tests. Plan 53-03 (POLISH-03 + POLISH-04) COMPLETE 2026-05-14: _subprocess_wrap_gsd_tools helper + amauta/bearings + amauta/agent-hydrate MCP tools appended to inline tools=[...] + 2 call_tool() handlers + 8 pytest tests (all pass). Plan 53-04 (POLISH-05) COMPLETE 2026-05-14: HYDRATE_CMD in cli-variables.md (LEARN-07 pattern) + GSD_HYDRATE_TASKS kill-switch hook injected before all Task() spawn sites (execute-phase-legacy.md 3 sites + step-03-execute.md 2 sites + step-04-verify.md 1 site) + 5 node:test tests (all pass). Plan 53-05 (closeout) COMPLETE 2026-05-14: tests/phase-53-canary.test.cjs (13/13 pass, NEVER SKIPS, SC1 byte-match 17/17 PASS, PHASE_53_BASE=32fbdc31) + REQUIREMENTS.md 22/22 Complete + 53-SUMMARY.md. Phase 53 COMPLETE — all POLISH-01..05 requirements fulfilled. v3.2 The Federation SHIPPED 2026-05-14.
