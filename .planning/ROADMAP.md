...
- [ ] **Phase 50: Party Mode Foundation** — `party_sessions` PG table + session state machine + persistent session memory (resumable from blackboard replay) (PARTY-01, PARTY-02). Plans 50-01 + 50-02 + 50-03 complete (2026-05-13): migration 021 (UP+DOWN) + PartySession Pydantic + 5 FROZEN transitions + post_finding() + list_findings() + resume() findings replay + party_session_cli.py + gsd-tools.cjs case 'party': + bin/cli.cjs party + 42 tests (32 Python + 10 Node). Plan 50-04 (E2E) remains.
