...
- [learning] 2026-04-14T12:24:55.877Z: Plan 39-03 pattern: eval framework Wave 3 — 15 code-based scenarios in tests/evals/ (5 per agent, grader_type code-based only in v3.0). eval-runner.cjs exits 0 when all pass. grader-schemas.json documents all 3 types (model-based/human for v3.1). Unit test (61 assertions, 5 groups) = pure fs reads per LIFE-XX group. Integration test (79 assertions) = per-describe spawnSync regression gates + cross-cutting identity loops over all 17 AGENT_FILES. Total v3.0: 820 assertions, 0 failures.
