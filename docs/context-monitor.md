...
| CRITICAL | <= 25% | Stop immediately, save state (`/amauta:pause-work`) |
...
GSD's `/amauta:pause-work` command saves execution state. The WARNING message suggests using it. The CRITICAL message instructs immediate state save.
