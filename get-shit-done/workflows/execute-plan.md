...
> **Contract (TK-2384):** `$PLAN_FILES` is a comma-separated **file list** — `src/App.tsx,services/api.py` — never a task description. A description matches no `file_pattern` and returns `executor-general`, which reads identically to "no specialist owns these files". The JSON answer carries `input_kind` (`file-list` | `mixed` | `not-a-file-list` | `empty`), `matched_agent`, `matched_pattern` and `reason`, so a caller can tell a match from a fallback; `.executor` is unchanged. An empty `$PLAN_FILES` returns `input_kind: "empty"` and is a hole in the plan's manifest, not a routing decision.
...
2. Per segment:
   - Subagent route: spawn gsd-executor-general for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/amauta:execute-phase {phase}` + `/amauta:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/amauta:plan-phase {Z+1}` + `/amauta:verify-work {Z}` + `/amauta:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/amauta:complete-milestone` + `/amauta:verify-work` + `/amauta:add-phase` |
