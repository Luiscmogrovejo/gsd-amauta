<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /amauta:plan-phase --gaps.
...
Provide a phase number to start testing (e.g., /amauta:verify-work 4)
```
...
- `/amauta:plan-phase {next}` — Plan next phase
- `/amauta:execute-phase {next}` — Execute next phase
```
</step>
...
<downstream_consumer>
Output consumed by /amauta:execute-phase
...
3. Abandon (exit, user runs /amauta:plan-phase manually)
...
`/clear` then `/amauta:execute-phase {phase} --gaps-only`
...
- [ ] If issues: gsd-checker verifies fix plans
- [ ] If issues: revision loop until plans pass (max 3 iterations)
- [ ] Ready for `/amauta:execute-phase --gaps-only` when complete
</success_criteria>
