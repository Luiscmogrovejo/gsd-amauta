<purpose>
Create all phases necessary to close gaps identified by `/amauta:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/amauta:add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/amauta:audit-milestone` first.
```
...
**Also available:**
- `/amauta:execute-phase {N}` — if plans already exist
...
`/amauta:audit-milestone` — re-audit to verify gaps closed
`/amauta:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/amauta:plan-phase` next
</success_criteria>
