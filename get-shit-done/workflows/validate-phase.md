...
```bash
AUDITOR_MODEL=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-validator --raw)
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /amauta:execute-phase {N} first."
...
## 5. Spawn gsd-validator

```
Task(
  prompt="Read ~/.claude/agents/gsd-validator.md for instructions.\n\n" +
