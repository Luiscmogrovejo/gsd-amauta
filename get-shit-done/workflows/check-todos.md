...
Todos are captured during work sessions with /amauta:add-todo.

---

Would you like to:

1. Continue with current phase (/amauta:progress)
2. Add a todo now (/amauta:add-todo)
```
...
- `/amauta:check-todos` → show all
- `/amauta:check-todos api` → filter to area:api only
</step>
...
- `/amauta:check-todos [area]` to filter by area
- `q` to exit
```
...
- options:
  - "Work on it now" — move to done, start working
  - "Create a phase" — /amauta:add-phase with this scope
  - "Brainstorm approach" — think through before deciding
  - "Put it back" — return to list
</step>
...
**Create a phase:**
Display: `/amauta:add-phase [description from todo]`
